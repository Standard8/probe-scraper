    ...
    if compress:
        ...
        with TemporaryDirectory() as tmp_name:
            ...
            if src.is_dir():
                ...
                if index.exists():
                    impl.push(
                        index, f"{dst}/{INDEX_HTML}", content_type=TEXT_HTML, **kwargs
                    )
                    kwargs["exclude"] = INDEX_HTML
                impl.push(
                    tmp, dst, content_type=APPLICATION_JSON, delete=delete, **kwargs
                )
            else:
                ...
                kwargs["content_type"] = (
                    TEXT_HTML if src.name == INDEX_HTML else APPLICATION_JSON
                )
